...
// Decodes the supplied video line and, if successful, returns the timecode data at *pRP188.
...
static void DoHalfTransition (uint8_t *pLine, uint32_t& pixelIndex, bool bBit0, bool bBit1)
{
	if (!bBit0 && !bBit1)
	{
		// 0 -> 0 transition: output 3 pixels of "low" level
		DoVITCPixel (pLine, pixelIndex++, VITC_YUV8_LO);
		DoVITCPixel (pLine, pixelIndex++, VITC_YUV8_LO);
		DoVITCPixel (pLine, pixelIndex++, VITC_YUV8_LO);
	}
	...
	{
		// 1 -> 1 transition: output 3 pixels of "high" level
		DoVITCPixel (pLine, pixelIndex++, VITC_YUV8_HI);
		DoVITCPixel (pLine, pixelIndex++, VITC_YUV8_HI);
		DoVITCPixel (pLine, pixelIndex++, VITC_YUV8_HI);
	}
