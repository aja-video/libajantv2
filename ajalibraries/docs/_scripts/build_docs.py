...
def main():
    ...
    if args.local is True and platform.node() == "mrbillmp.aja.com":
        ...
        aja.utils.copy_filelist(
            os.path.join("ajalibraries/docs"),
            build_dir,
        ...
        copied_items = aja.utils.copy_filelist(
            os.path.join("."), build_dir, ["ntv2sdk*.zip"], False, "3"
        )
        ...
        if len(copied_items) != 3:
            print(
                "## ERROR:  %d SDK .zip file(s) copied, should be 3"
                % (len(copied_items))
            )
            print(copied_items)
            return 502
    ...
    stuff_to_unzip = [
        {"name": "MacSDK", "rename": "ntv2sdkmac_*.zip", "to": "ntv2sdkmac.zip"},
    ...
    major = aja.utils.get_defined_macro_value(
        "ajalibraries/ajantv2/includes/ntv2enums.h", "AJA_NTV2_SDK_VERSION_MAJOR"
    )
    minor = aja.utils.get_defined_macro_value(
        "ajalibraries/ajantv2/includes/ntv2enums.h", "AJA_NTV2_SDK_VERSION_MINOR"
    )
    point = aja.utils.get_defined_macro_value(
        "ajalibraries/ajantv2/includes/ntv2enums.h", "AJA_NTV2_SDK_VERSION_POINT"
    )
    build_num = aja.utils.get_defined_macro_value(
        "ajalibraries/ajantv2/includes/ntv2enums.h", "AJA_NTV2_SDK_BUILD_NUMBER"
    )
    build_when = aja.utils.get_defined_macro_value(
        "ajalibraries/ajantv2/includes/ntv2enums.h", "AJA_NTV2_SDK_BUILD_DATETIME"
    )
    build_type = aja.utils.get_defined_macro_value(
        "ajalibraries/ajantv2/includes/ntv2enums.h", "AJA_NTV2_SDK_BUILD_TYPE"
    )
    if major != expected_major or minor != expected_minor or point != expected_point:
        print(
            "## ERROR:  SDK version %s.%s.%s doesn't match expected version %s.%s.%s"
            % (major, minor, point, expected_major, expected_minor, expected_point)
        )
        return 502
    ...
    if build_type == "":
        ...
        underscore_version = "%s_%s_%s_%s" % (major, minor, point, build_num)
        print(
            "## NOTE:  Documenting SDK %s.%s.%s build %s built on '%s'"
            % (major, minor, point, build_num, build_when)
        )
    else:
        display_version = "%s.%s.%sb%s" % (major, minor, point, build_num)
        underscore_version = "%s_%s_%sbeta" % (major, minor, point)
        print(
            "## NOTE:  Documenting SDK %s.%s.%s build %s (beta) built on '%s' -- underscore_version='%s'"
            % (major, minor, point, build_num, build_when, underscore_version)
        )

    # Grab Windows SDK build numbers...
    win_major = aja.utils.get_defined_macro_value(
        "winntv2enums.h", "AJA_NTV2_SDK_VERSION_MAJOR"
    )
    win_minor = aja.utils.get_defined_macro_value(
        "winntv2enums.h", "AJA_NTV2_SDK_VERSION_MINOR"
    )
    win_point = aja.utils.get_defined_macro_value(
        "winntv2enums.h", "AJA_NTV2_SDK_VERSION_POINT"
    )
    win_build_num = aja.utils.get_defined_macro_value(
        "winntv2enums.h", "AJA_NTV2_SDK_BUILD_NUMBER"
    )
    win_build_when = aja.utils.get_defined_macro_value(
        "winntv2enums.h", "AJA_NTV2_SDK_BUILD_DATETIME"
    )
    win_build_type = aja.utils.get_defined_macro_value(
        "winntv2enums.h", "AJA_NTV2_SDK_BUILD_TYPE"
    )
    if major != win_major or minor != win_minor or point != win_point:
        print(
            "## ERROR:  SDK version %s.%s.%s doesn't match Windows version %s.%s.%s"
            % (major, minor, point, win_major, win_minor, win_point)
        )
        return 502

    # Grab Linux SDK build numbers...
    lin_major = aja.utils.get_defined_macro_value(
        "linntv2enums.h", "AJA_NTV2_SDK_VERSION_MAJOR"
    )
    lin_minor = aja.utils.get_defined_macro_value(
        "linntv2enums.h", "AJA_NTV2_SDK_VERSION_MINOR"
    )
    lin_point = aja.utils.get_defined_macro_value(
        "linntv2enums.h", "AJA_NTV2_SDK_VERSION_POINT"
    )
    lin_build_num = aja.utils.get_defined_macro_value(
        "linntv2enums.h", "AJA_NTV2_SDK_BUILD_NUMBER"
    )
    lin_build_when = aja.utils.get_defined_macro_value(
        "linntv2enums.h", "AJA_NTV2_SDK_BUILD_DATETIME"
    )
    lin_build_type = aja.utils.get_defined_macro_value(
        "linntv2enums.h", "AJA_NTV2_SDK_BUILD_TYPE"
    )
    if major != lin_major or minor != lin_minor or point != lin_point:
        print(
            "## WARNING:  SDK version %s.%s.%s doesn't match Linux version %s.%s.%s"
            % (major, minor, point, lin_major, lin_minor, lin_point)
        )
        # return 502
    ...
    if args.local is True and platform.node() == "mrbillmp.aja.com":
        ...
        print(
            "Local dev build ... replace ajalibraries/ajabase with dev tree's ajabase folder..."
        )
        shutil.rmtree(os.path.join("ajalibraries", "ajabase"))
        copied_items = aja.utils.copy_filelist(
            os.path.join("..", "ajalibraries"),
            os.path.join("ajalibraries"),
            ["ajabase"],
            False,
            "3",
        ...
        print(
            "Local dev build ... replace ajalibraries/ajaanc with dev tree's ajaanc folder..."
        )
        ...
        print(
            "Local dev build ... replace ajalibraries/ajacc with dev tree's ajacc folder..."
        )
        ...
        print(
            "Local dev build ... replace ajalibraries/ajantv2 with dev tree's ajantv2 folder..."
        )
    ...
    stuff_to_patch = [
        {
            "name": "Versionize Doxy Config File",
    ...
    result_code = aja.utils.stage(
        [
            {
                "name": "Rename HTML Folder",
                "rename": "html",
                "from": "",
                "to": html_folder_path,
            },
    ...
    if result_code == 0:
        print(
            "## NOTE:  Documentation set should be accessible using URL:  %s"
            % (client_browser_url)
        )
    else:
        print(
            "## ERROR:  Documentation failed to rsync to '%s' due to error %d"
            % (destination_rsync_url, result_code)
        )
    return 0
