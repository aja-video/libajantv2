...
public:
    ...
    DpxHdr ( const char* path );
           

    DpxHdr ( const DpxHdr& rhs );
    DpxHdr ( void *buffer, size_t buffer_len=2048 );

    DpxHdr operator=( const void *buffer );
    ...
    size_t get_ii_image_size() 
        { return get_ii_pixels() * get_ii_lines() * 4; }
