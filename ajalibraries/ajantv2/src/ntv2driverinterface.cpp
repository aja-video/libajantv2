...
bool CNTV2DriverInterface::OpenRemote (const UWord inDeviceIndex, const string & inHostName)
{
#ifdef AJA_WINDOWS
    initWinsock();
#endif
	...
	string oldName(GetHostName()), newName(inHostName);
	aja::lower(oldName);  aja::lower(aja::strip(newName));
	if (IsRemote()  &&  oldName == newName)
		DIINFO("Already connected to '" << newName << "':" << *_pRPCAPI);
	else
	{
		CloseRemote();
		//	Open the remote device...
		_pRPCAPI = NTV2RPCAPI::MakeNTV2FakeDevice(newName, 0);
		if (!_pRPCAPI)	//	Try again?
			_pRPCAPI = NTV2RPCAPI::MakeNTV2NubRPCAPI(newName, inDeviceIndex);
	}
	return IsRemote();
}
...
#if defined (NTV2_NUB_CLIENT_SUPPORT)
	if (IsRemote())
		return whichRegisterFailed ? !_pRPCAPI->NTV2ReadRegisterMultiRemote(numRegs, *whichRegisterFailed, aRegs) : false;
#endif	//	defined (NTV2_NUB_CLIENT_SUPPORT)
										...
										const bool			inSynchronous)
{
	...
	return !_pRPCAPI->NTV2DMATransferRemote(inDMAEngine, inIsRead, inFrameNumber, pFrameBuffer, inCardOffsetBytes, inByteCount,
											0/*numSegments*/,  0/*segHostPitch*/,  0/*segCardPitch*/, inSynchronous);
#else
	(void) inDMAEngine;
	(void) inIsRead;
	(void) inFrameNumber;
									...
									const bool inSynchronous)
{
	...
	return !_pRPCAPI->NTV2DMATransferRemote(inDMAEngine, inIsRead, inFrameNumber, pFrameBuffer, inCardOffsetBytes,
											inByteCount, inNumSegments, inSegmentHostPitch, inSegmentCardPitch, inSynchronous);
#else
	(void) inDMAEngine;
	(void) inIsRead;
	(void) inFrameNumber;
...
#if defined (NTV2_NUB_CLIENT_SUPPORT)
	NTV2_ASSERT(IsRemote());
	return !_pRPCAPI->NTV2WriteRegisterRemote(registerNumber, registerValue, registerMask, registerShift);
#else
	(void) registerNumber;
	(void) registerValue;
	(void) registerMask;
