...
class RoutingExpert
{
	...
	public:
		...
		string				InputXptToString (const NTV2InputXptID inInputXpt) const
		{
		...
		string				OutputXptToString (const NTV2OutputCrosspointID inOutputXpt) const
		{
			...
			return iter != gOutputXpt2String.end() ? iter->second : string();
		}

		NTV2InputXptID		StringToInputXpt (const string & inStr) const
		{
			AJAAutoLock	lock(&gLock);
			NTV2_ASSERT(!gString2InputXpt.empty ());
			string	str(inStr);	aja::lower(aja::strip(str));
		...
		NTV2OutputXptID		StringToOutputXpt (const string & inStr) const
		{
			AJAAutoLock	lock(&gLock);
...
void RoutingExpert::InitInputXpt2String(void)
{
	...
	{
		string	lowerStr(iter->second);	aja::lower(lowerStr);
...
bool CNTV2SignalRouter::CreateFromString (const string & inString, CNTV2SignalRouter & outRouter)	//	STATIC
{
    ...
    for (NTV2StringListConstIter pEachLine(lines.begin());  pEachLine != lines.end();  ++pEachLine)
    {
		...
		if (!outRouter.AddConnection(inputXpt, outputXpt))
			{SRFAIL("AddConnection('" << leftPiece << "', '" << rightPiece << "') failed");  return false;}
    }	//	for each line
    return true;
}
