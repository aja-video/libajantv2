...
const char *NTV2NubQueryRespStrProtVer3[eNumNTV2NubPktTypes] =
{
	// The discover strings must be the same for all protocol versions
	// so that different versions of watchers and nubs can find each other.
	"Our chief weapons are?",
...
const char *
nubQueryRespStr(NTV2NubProtocolVersion	protocolVersion,
				NTV2NubPktType			pktType)
{
	const char *queryRespStr = 0;
...
{
	const char *queryRespStr = nubQueryRespStr(pPkt->hdr.protocolVersion, 
												pPkt->hdr.pktType);
	char *p = (char *)pPkt->data;	
...
// Build a nub packet and fill in all the common stuff for the
// given protocol and type.
NTV2NubPkt *
BuildNubBasePacket( NTV2NubProtocolVersion	protocolVersion,
					NTV2NubPktType			pktType,
					ULWord					payloadSize,
					char 					**pPayload)
{
	...
	char *p = (char *)pPkt->data;	// Work around ISO C++ forbids zero-size array
...
void 
dumpBoardInventory(NTV2DiscoverRespPayload *boardInventory)
{
...
bool isNTV2NubPacketType(NTV2NubPkt *pPkt, NTV2NubPktType nubPktType)
{
	...
	switch(pPkt->hdr.protocolVersion)
	{
		case ntv2NubProtocolVersion1:
			...
			if (fixedPktType == pPkt->hdr.pktType
				&&	!strncmp((const char *)pPkt->data, 
					NTV2NubQueryRespStrProtVer1[nubPktType], 
					pPkt->hdr.dataLength)
				)
			{
				return true;
		...
		case ntv2NubProtocolVersion2:
			return (pPkt->hdr.pktType == nubPktType 
				&&	!strncmp((const char *)pPkt->data, 
					NTV2NubQueryRespStrProtVer2[nubPktType], 
					pPkt->hdr.dataLength));
		...
		default:	
			return (pPkt->hdr.pktType == nubPktType 
				&&	!strncmp((const char *)pPkt->data, 
					NTV2NubQueryRespStrProtVer3[nubPktType], 
					pPkt->hdr.dataLength));
	}
}
