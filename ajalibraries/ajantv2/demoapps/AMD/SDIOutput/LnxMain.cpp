...
void Resize(unsigned int uiWidth, unsigned int uiHeight)
{
    ...
    if (g_pSource)
    {
        g_pSource->resize(uiWidth, uiHeight);
    }
}
...
int main(int argc, char** argv)
{
    ...
    {
         std::cerr<<"\nCould not configure SDI output!"<<std::endl;
         return 0;
    }
    ...
    if (bPinnedMemory && bBusAddressableMemory)
    {
        ...
        // Create a stream to upload the data from the gpu to pinned memory
        // This function will create a buffer that contains the frames that is synchronized
        ...
        // Since channel 0 was configured as output it will use the buffer
        // as input, upload it to the SDI framebuffer and display it
