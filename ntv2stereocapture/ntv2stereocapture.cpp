...
NTV2StereoCapture::~NTV2StereoCapture ()
{
	...
	mDevice.ReleaseStreamForApplication(NTV2_FOURCC('D','E','M','O'), static_cast<int32_t>(AJAProcess::GetPid()));
...
AJAStatus NTV2StereoCapture::Init (const CNTV2DeviceScanner & inBoardScanner, const uint32_t inBoardNumber)
{
	...
	if (!mDevice.AcquireStreamForApplication(NTV2_FOURCC('D','E','M','O'), static_cast<int32_t>(AJAProcess::GetPid())))
	{
		mDevice.Close();
