...
static const ULWord gHdrFirst = 330;
static const ULWord gHdrLast = 336;
static ULWord gHdrReg[7] = { 0, 0, 0, 0, 0, 0, 0 };

static const ULWord gChannelTo1DLutControlRegNum []	=
{ kReg1DLUTLoadControl1, kReg1DLUTLoadControl2, kReg1DLUTLoadControl3, kReg1DLUTLoadControl4,
  kReg1DLUTLoadControl5,	kReg1DLUTLoadControl6,	kReg1DLUTLoadControl7,	kReg1DLUTLoadControl8,	0};

static bool ntv2_isLutLoad(ULWord reg, ULWord val, ULWord mask, ULWord shift)
{
    int index = 0;
    ULWord lutReg = 0;
    ...
    while (gChannelTo1DLutControlRegNum[index] != 0)
    {
        if (reg == gChannelTo1DLutControlRegNum[index])
        {
            lutReg = reg;
            break;
        }
...
#  endif
        ...
        if (lutLoad)
        {
            printk(KERN_ALERT "lut load trigger done\n");
        }
        if (dmaHardware)
        {
            msleep(1);
            dmaHardwareRelease(getDmaEngine(deviceNumber, 0));
//            printk("lut load released dma engine\n");
        }
