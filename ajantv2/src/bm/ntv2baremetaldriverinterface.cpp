...
/**
	@file		ntv2baremetaldriverinterface.cpp
	@brief		Implementation of the CNTV2BareMetalDriverInterface class.
	@copyright	(C) 2003-2022 AJA Video Systems, Inc.
**/
#include "ntv2baremetaldriverinterface.h"
#include "ntv2baremetalpublicinterface.h"
#include "ntv2utils.h"
...
//	BareMetalDriverInterface Logging Macros
...
/////////////////////////////////////////////////////////////////////////////////////
bool CNTV2BareMetalDriverInterface::OpenLocalPhysical (const UWord inDeviceIndex)
{
...
bool CNTV2BareMetalDriverInterface::CloseLocalPhysical (void)
{
#if 0
	NTV2_ASSERT(!IsRemote());
	NTV2_ASSERT(IsOpen());
...
bool CNTV2BareMetalDriverInterface::ReadRegister (const ULWord inRegNum,  ULWord & outValue,  const ULWord inMask,	const ULWord inShift)
{
	if (inShift >= 32)
	{
  ...
  outValue = (*(uint32_t *)(0x80000000+4*inRegNum) & inMask) >> inShift;
//  printf("read reg - 0x%08lX = 0x%08lX (0x%08lX) %08lX/%d\n", inRegNum,
//      *(uint32_t *)(4*inRegNum), outValue, inMask, inShift);
	return true;
}


bool CNTV2BareMetalDriverInterface::WriteRegister (const ULWord inRegNum,  const ULWord inValue,  const ULWord inMask, const ULWord inShift)
{
	if (inShift >= 32)
	{
		LDIFAIL("Shift " << DEC(inShift) << " > 31, reg=" << DEC(inRegNum) << " msk=" << xHEX0N(inMask,8));
  ...
  uint32_t val = *(uint32_t *)(0x80000000+4*inRegNum);
  val &= ~inMask;
  val |= (inValue << inShift) & inMask;
//  printf("write reg - 0x%08lX at 0x%08lX = 0x%08lX (0x%08lX) %08lX/%d\n", inRegNum,
//        4*inRegNum, val, inValue, inMask, inShift);
  *(uint32_t *)(0x80000000+4*inRegNum) = val;
	return true;
}

bool CNTV2BareMetalDriverInterface::RestoreHardwareProcampRegisters (void)
{
#if 0
	if (IsRemote())
		return false;
...
bool CNTV2BareMetalDriverInterface::ConfigureInterrupt (const bool bEnable, const INTERRUPT_ENUMS eInterruptType)
{
#if 0
	NTV2_ASSERT( (_hDevice != INVALID_HANDLE_VALUE) && (_hDevice != 0) );
...
bool CNTV2BareMetalDriverInterface::GetInterruptCount (const INTERRUPT_ENUMS eInterruptType, ULWord & outCount)
{
#if 0
	NTV2_ASSERT( (_hDevice != INVALID_HANDLE_VALUE) && (_hDevice != 0) );
...
bool CNTV2BareMetalDriverInterface::WaitForInterrupt (const INTERRUPT_ENUMS eInterrupt, const ULWord timeOutMs)
{
#if 0
	if (IsRemote())
...
// Output: True on successs, false on failure (ioctl failed or interrupt didn't happen)
bool CNTV2BareMetalDriverInterface::ControlDriverDebugMessages (NTV2_DriverDebugMessageSet msgSet, bool enable)
{
#if 0
	NTV2_ASSERT( (_hDevice != INVALID_HANDLE_VALUE) && (_hDevice != 0) );
...
// Method: SetupBoard
// Output: True on successs, false on failure (ioctl failed or interrupt didn't happen)
bool CNTV2BareMetalDriverInterface::SetupBoard (void)
{
#if 0
	NTV2_ASSERT( (_hDevice != INVALID_HANDLE_VALUE) && (_hDevice != 0) );
...
//////////////////////////////////////////////////////////////////////////////
	...
	bool CNTV2BareMetalDriverInterface::MapFrameBuffers (void)
	{
#if 0
		if (!IsOpen())
			return false;
	...
	// Input:	None
	// Output:	bool status
	bool CNTV2BareMetalDriverInterface::UnmapFrameBuffers (void)
	{
#if 0
		if (!_pFrameBaseAddress)
			return true;
	...
	// Input:	None
	// Output:	bool, and sets member _pBaseFrameAddress
	bool CNTV2BareMetalDriverInterface::MapRegisters (void)
	{
#if 0
		if (!IsOpen())
			return false;
	...
	// Input:	None
	// Output:	bool status
	bool CNTV2BareMetalDriverInterface::UnmapRegisters (void)
	{
#if 0
		if (!IsOpen())
			return false;
	...
	bool CNTV2BareMetalDriverInterface::GetBA0MemorySize (ULWord* memSize)
	{
		return memSize ? ReadRegister (kVRegBA0MemorySize, *memSize) : false;
	}

	bool CNTV2BareMetalDriverInterface::GetBA1MemorySize (ULWord* memSize)
	{
		return memSize ? ReadRegister (kVRegBA1MemorySize, *memSize) : false;
	}

	bool CNTV2BareMetalDriverInterface::GetBA2MemorySize (ULWord* memSize)
	{
		return memSize ? ReadRegister (kVRegBA2MemorySize, *memSize) : false;
	}

	bool CNTV2BareMetalDriverInterface::GetBA4MemorySize (ULWord* memSize)
	{
		return memSize ? ReadRegister (kVRegBA4MemorySize, *memSize) : false;
	}

	bool CNTV2BareMetalDriverInterface::MapXena2Flash (void)
	{
#if 0
		if (!IsOpen())
			return false;
	...
	bool CNTV2BareMetalDriverInterface::UnmapXena2Flash (void)
	{
#if 0
		if (!_pXena2FlashBaseAddress)
			return true;
	...
	bool CNTV2BareMetalDriverInterface::MapDNXRegisters (void)
	{
#if 0
	...
	bool CNTV2BareMetalDriverInterface::UnmapDNXRegisters (void)
	{
#if 0
		if (!_pDNXRegisterBaseAddress)
			return true;
...
bool CNTV2BareMetalDriverInterface::DmaTransfer (	const NTV2DMAEngine inDMAEngine,
												const bool			inIsRead,
...
{
  uint8_t *mem = (uint8_t *)*(uint32_t *)(0x80000000+4*0x3442);
  printf("%08lx\n", mem);
  memcpy(mem, pFrameBuffer, inByteCount);
#if 0
	if (IsRemote())
											...
											const bool				inIsSynchronous)
{
	...
	// NOTE: BareMetal driver assumes driver buffers to be used if pFrameBuffer < numDmaDriverBuffers
...
///////////////////////////////////////////////////////////////////////////
// AutoCirculate
bool CNTV2BareMetalDriverInterface::AutoCirculate (AUTOCIRCULATE_DATA & autoCircData)
{
#if 0
	if (IsRemote())
...
bool CNTV2BareMetalDriverInterface::NTV2Message (NTV2_HEADER * pInMessage)
{
#if 0
	if (!pInMessage)
	...
	bool CNTV2BareMetalDriverInterface::GetDMANumDriverBuffers (ULWord* pNumDmaDriverBuffers)
	{
	...
	// Input:
	// Output:
	bool CNTV2BareMetalDriverInterface::MapDMADriverBuffer (void)
	{
#if 0
	...
	bool CNTV2BareMetalDriverInterface::GetDMADriverBufferAddress (ULWord** pDMADriverBufferAddress)
	{
#if 0
		if (!_pDMADriverBufferAddress)
	...
	// Input:  NONE
	// Output: NONE
	bool CNTV2BareMetalDriverInterface::UnmapDMADriverBuffer (void)
	{
#if 0
		if (_pDMADriverBufferAddress)
		{
...
bool CNTV2BareMetalDriverInterface::SetAudioOutputMode (NTV2_GlobalAudioPlaybackMode mode)
{
	return WriteRegister(kVRegGlobalAudioPlaybackMode,mode);
}

bool CNTV2BareMetalDriverInterface::GetAudioOutputMode (NTV2_GlobalAudioPlaybackMode* mode)
{
	...
	bool CNTV2BareMetalDriverInterface::DmaWriteFrameDriverBuffer (NTV2DMAEngine DMAEngine, ULWord frameNumber, unsigned long dmaBufferFrame, ULWord bytes, ULWord poll)
	{
#if 0
		if (IsRemote())
			return false;
	...
	// This allows dma's to be done without scatter/gather
	// which should help performance.
	bool CNTV2BareMetalDriverInterface::DmaWriteFrameDriverBuffer (NTV2DMAEngine DMAEngine, ULWord frameNumber, unsigned long dmaBufferFrame, ULWord offsetSrc, ULWord offsetDest, ULWord bytes, ULWord poll)
	{
#if 0
		if (IsRemote())
			return false;
	...
	// This allows dma's to be done without scatter/gather
	// which should help performance.
	bool CNTV2BareMetalDriverInterface::DmaReadFrameDriverBuffer (NTV2DMAEngine DMAEngine, ULWord frameNumber, unsigned long dmaBufferFrame, ULWord bytes, ULWord downSample, ULWord linePitch, ULWord poll)
	{
#if 0
		if (IsRemote())
			return false;
	...
	// This allows dma's to be done without scatter/gather
	// which should help performance.
	bool CNTV2BareMetalDriverInterface::DmaReadFrameDriverBuffer (NTV2DMAEngine DMAEngine, ULWord frameNumber, unsigned long dmaBufferFrame,
...
bool CNTV2BareMetalDriverInterface::DmaWriteWithOffsets (NTV2DMAEngine DMAEngine, ULWord frameNumber, ULWord * pFrameBuffer,
													ULWord offsetSrc, ULWord offsetDest, ULWord bytes)
{
#if 0
	...
	if (!IsOpen())
		return false;
	// NOTE: BareMetal driver assumes driver buffers to be used if
...
bool CNTV2BareMetalDriverInterface::DmaReadWithOffsets (NTV2DMAEngine DMAEngine, ULWord frameNumber, ULWord * pFrameBuffer,
													ULWord offsetSrc, ULWord offsetDest, ULWord bytes)
{
#if 0
	// return DmaTransfer (DMAEngine, false, frameNumber, pFrameBuffer, (ULWord) 0, bytes, bSync);
